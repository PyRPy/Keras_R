...
path <- get_file(
  "artofwar.txt",
  origin = "https://www.gutenberg.org/files/132/132.txt"
)

text <- tolower(readChar(path, file.info(path)$size))
cat("Corpus length:", nchar(text), "\n")

maxlen <- 20  # Length of extracted character sequences
...
# train the model and sampling from it
...
# the following loop repeatedly trains and generates text
# This allows you to see how the generated text evolves as the model begins to 
# converge, as well as the impact of temperature in the sampling strategy.
